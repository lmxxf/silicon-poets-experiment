...
### 踩坑修正：「鹜」被 BPE 拆成字节，不是 bug

跑「落霞与孤鹜齐飞」时，分析输出里「鹜」位置出现两个乱码 `�`，一度怀疑模型/PyTorch/tokenizer 出 bug。查证后确认**不是 bug**：

- 「鹜」（野鸭）生僻，DeepSeek tokenizer 词表里没有独立 token，BPE 把它按字节拆成两个 token：`e9b9` + `9c`，拼起来正是「鹜」的 UTF-8（`e9b99c`）。
- 单独对每个字节 token `decode([id])` 得到半截字节，非法 UTF-8 → 显示为 `�`。模型数值全对，只是**逐 token 解码的显示问题**。
- 与 vLLM 那次 Marlin 英文乱码（kernel 数值真错）本质不同——这次是分词机制，不是 kernel bug。

`probe_all_analyze.py` 已加字节碎片合并解码（`display_tokens`），从 `input_ids` 重建完整字，王勃组「鹜=0.09」正常显示。
